...
async def generate_friend_reply(chat_id: Optional[str], message: str, email: str) -> tuple[str, str, Optional[str]]:
    ...
    # Try Gemini first
    try:
        print("[Friend Chat] Calling Gemini...")
        ...
        print(f"[Friend Chat] Gemini succeeded ({len(reply)} chars)")
        
    except Exception as gemini_error:
        print(f"[Friend Chat] Gemini failed: {str(gemini_error)[:100]}")
        
        # Fallback to Groq
        try:
            print("[Friend Chat] Falling back to Groq...")
            ...
            provider_used = "groq"
            print(f"[Friend Chat] Groq succeeded ({len(reply)} chars)")
            
        except Exception as groq_error:
            print(f"[Friend Chat] Groq also failed: {str(groq_error)[:100]}")
    ...
    if provider_used and provider_used != "none":
        ...
        print(f"[Friend Chat] Response length: {len(reply) if reply else 0} characters")
